#' ---- Benchmark supercell on Samusik all data ----
#' Essentially:
...
library(HDCytoData) # only for preparing the data
library(data.table)
...
# Main directories
directories <- list(
    data_savedir = here("data", "explore_supercell_purity_clustering", "samusik_all"),
    res_savedir = here("output", "explore_supercell_purity_clustering", format(Sys.Date(), "%Y%m%d"), "samusik_all")
)
...
prepare_data_samusik(data_savedir)

# ---- 1. Run supercell ----

run_supercell(
    cell_dat_file = here(directories$data_savedir, "samusik_all_asinh.csv"),
    marker_info_file = here(directories$data_savedir, "samusik_all_asinh_markers_info.csv"),
    gamma_values = gamma_values,
    save_dir = directories$supercell_res_savedir,
    n_parallel_worker = parallel::detectCores() - 1
)

# ---- 2. Cluster supercells using Louvain ----

# Setup configs
marker_info <- fread(here(directories$data_savedir, "samusik_all_asinh_markers_info.csv"))
...
for (gamma_value in gamma_values) {
    ...
    message(paste("clustering supercell gamma", gamma_value))
...
fwrite(
    x = calc_ari_supercell_allcell_flowsom(
        ground_truth_file = here(directories$data_savedir, "samusik_all_asinh.csv"),
        ...
        clust_allcell_res_dir = here("output", "explore_supercell_purity_clustering", "20230509", "samusik_all", "flowsom_allcells")
    ),
